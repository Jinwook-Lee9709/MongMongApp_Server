...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); // 1. 폼 제출 시 페이지가 새로고침되는 것을 방지
    try {
      // 2. 서버에 로그인 요청을 보냄
      const response = await axios.post(
        "http://localhost:3000/api/auth/login", // 요청을 보낼 서버의 주소
        formData, // 3. 사용자가 입력한 아이디, 비밀번호 등의 데이터
        {
          withCredentials: true, // 4. 쿠키를 주고받기 위한 중요한 설정
        }
      );
      // 5. 서버로부터 받은 응답을 확인
